...
void test_ut_kvp_get_instance( void )
{
    ...
    if(resultP2 != true)
    {
        UT_LOG_ERROR("perhaps platform profile(assets/include/depth_check.yaml) was not passed using -p switch");
    }
    UT_LOG_STEP("Tested for profile : assets/include/depth_check.yaml");
}
